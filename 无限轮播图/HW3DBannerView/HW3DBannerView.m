...
-(instancetype)initWithFrame:(CGRect)frame{
    if (self = [super initWithFrame:frame]) {
        self.imgWidth = ZXMainScrollViewWidth; // 设置图片宽度
        [self initialization];// 设置一些参数
        [self setUpUI];// 创建页面 左边 中间 右边的图片视图
        
    }
    return self;
}
#pragma mark - 外部初始化滑动器
+(instancetype)initWithFrame:(CGRect)frame
...
-(void)initialization{
    ...
    _imageHeightPoor = 0;
    self.otherPageControlColor = [UIColor grayColor]; //其他原点颜色
    self.curPageControlColor = [UIColor whiteColor];// 当前原点颜色
...
// 创建三个图片视图
-(void)setUpUI{
    [self addSubview:self.mainScrollView]; // View上面添加一个滑动视图
    //图片视图；左边
    ...
    [self updateViewFrameSetting]; // 设置图片尺寸
}
#pragma mark - 设置中间卡片与两边卡片的高度差
- (void)setImageHeightPoor:(CGFloat)imageHeightPoor {
    _imageHeightPoor = imageHeightPoor;
    [self updateViewFrameSetting];// 设置初始尺寸
}
...
- (void)setData:(NSArray *)data {
    ...
    [self setInfoByCurrentImageIndex:self.currentImageIndex]; // 赋值
    ...
    [self createPageControl]; // 创建页码控制器
}
...
- (void)reloadImage {
    ...
    [self setInfoByCurrentImageIndex:_currentImageIndex]; // 给图片赋值
}
...
#pragma mark - 定时器绑定的自动滚动的方法
- (void)automaticScroll {
    if (0 == _imgCount) return;
    if(self.mainScrollView.scrollEnabled == NO) return;
    [self.mainScrollView setContentOffset:CGPointMake(ZXMainScrollViewWidth*2, 0.0) animated:YES]; // 跳转到指定的坐标
}

#pragma mark -- properties属性操作
